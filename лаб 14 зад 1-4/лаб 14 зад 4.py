...
def tabulate_function():
    ...
    label_function_formula.config(text='y=1+ln(2*x)')  # Обновление метки для отображения формулы
    ...
    for i in range(iterations):
        ...
        y = y=1+math.log(2*x)  # Вычисление значения функции
        result = f'   {x:.2f}              {y:.2f}'  # Форматирование результата
        listbox.insert(END, result)  # Всталяем результат в список
        progress['value'] = i + 1  # Обновляем значения прогресс-бара
...
label_function_formula = Label(window, text='y=1+ln(2*x)', font=('Arial', 12))
...
progress.pack()
# Создание кнопки для запуска процесса табулирования
